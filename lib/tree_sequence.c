...
tree_sequence_dump(tree_sequence_t *self, const char *filename, int flags)
{
    ...
    ret = tree_sequence_dump_tables(self, &tables.nodes, &tables.edges,
            &tables.migrations, &tables.sites, &tables.mutations,
            &tables.provenances, MSP_ALLOC_TABLES);
    if (ret != 0) {
        goto out;
    }
    tables.sequence_length = self->sequence_length;
    ret = table_collection_dump(&tables, filename, flags);
out:
    table_collection_free(&tables);
    return ret;
}
